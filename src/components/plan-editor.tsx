...
export function PlanEditor() {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    setMessage("");
    try {
      const response = await fetch("/api/plan", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ name: data.get("name"), date: data.get("date"), type: "expense", amountCents: Math.round(Number(data.get("amount")) * 100) }) });
  ...
  return <section className="plan-add-section">
    ...
    {open ? <form className="plan-editor card" onSubmit={submit}>
      ...
      <div className="field"><label htmlFor="plan-date">Expected date</label><input id="plan-date" name="date" type="date" min={format(new Date(), "yyyy-MM-dd")} required /></div>
