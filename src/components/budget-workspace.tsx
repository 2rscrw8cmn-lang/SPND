...
export function BudgetWorkspace({ initialWorkspace }: { initialWorkspace: Workspace }) {
  ...
  return <>
    ...
    <section className="budget-summary" aria-label="Monthly budget summary"><div><span>Spent this month</span><strong>{formatCurrency(totals.spent)}</strong><small>{formatCurrency(totals.pending)} pending</small></div><div className="summary-progress"><span style={{ width: `${Math.min(100, totals.budgeted ? totals.spent / totals.budgeted * 100 : 0)}%` }} /></div></section>
