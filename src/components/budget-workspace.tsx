...
export function BudgetWorkspace({ initialWorkspace }: { initialWorkspace: Workspace }) {
  ...
  return <>
    ...
    {alert ? <button className="budget-alert" onClick={() => setSelectedId(alert.id)}><span><TriangleAlert size={17} /><span>{alert.name} is {formatCurrency(alert.spentCents + alert.pendingCents - alert.budgetedCents)} over budget</span></span><strong>Review <ChevronRight size={16} /></strong></button> : null}
    ...
    {selected ? <CategoryPanel category={selected} categoryGroups={categoryGroups} month={initialWorkspace.month} message={message} onClose={() => setSelectedId(null)} onSaveCategory={saveCategory} onSaveBudget={saveBudget} /> : null}
  </>;
}
...
function CategoryPanel({ category, categoryGroups, month, message, onClose, onSaveCategory, onSaveBudget }: { category: BudgetCategory; categoryGroups: string[]; month: string; message: string; onClose: () => void; onSaveCategory: (value: BudgetCategory) => Promise<boolean>; onSaveBudget: (id: string, cents: number) => Promise<boolean> }) {
