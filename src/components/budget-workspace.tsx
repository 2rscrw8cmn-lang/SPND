...
export function BudgetWorkspace({ initialWorkspace }: { initialWorkspace: Workspace }) {
  ...
  const incomeBasisCents = expectedIncomeCents > 0 ? expectedIncomeCents : initialWorkspace.totals.receivedIncomeCents;
  ...
  return <>
    ...
    <section className="budget-summary" aria-label="Monthly budget summary"><div className="budget-summary-copy"><span>{format(monthDate, "MMMM")} left to assign</span><strong className={available < 0 ? "negative" : ""}>{formatCurrency(available, { signed: available < 0 })}</strong><small>{incomeBasisLabel} income minus assigned</small></div><div className="budget-summary-ring" style={{ "--spent": `${Math.min(100, totals.budgeted ? (totals.spent + totals.pending) / totals.budgeted * 100 : 0)}%` } as React.CSSProperties}><div><strong>{formatCurrency(totals.spent, { compact: true })}</strong><span>of {formatCurrency(totals.budgeted, { compact: true })}</span><small>spent</small></div></div></section>
    <dl className="budget-metric-strip budget-income-strip"><div className="income-metric-link"><dt><Link href={`/income?month=${format(monthDate,"yyyy-MM")}`}>Expected income <ChevronRight size={14}/></Link></dt><dd>{formatCurrency(initialWorkspace.totals.expectedIncomeCents, { compact: true })}</dd></div><div><dt>Received</dt><dd>{formatCurrency(initialWorkspace.totals.receivedIncomeCents, { compact: true })}</dd></div><div><dt>Expected remaining</dt><dd>{formatCurrency(initialWorkspace.totals.remainingExpectedIncomeCents, { compact: true })}</dd></div><div><dt>Assigned</dt><dd>{formatCurrency(totals.budgeted, { compact: true })}</dd></div><div><dt>Left to assign</dt><dd className={available < 0 ? "negative" : ""}>{formatCurrency(available, { compact: true, signed: available < 0 })}</dd></div><div><dt>Spent · Pending</dt><dd>{formatCurrency(totals.spent, { compact: true })} · {formatCurrency(totals.pending, { compact: true })}</dd></div></dl>
    {incomeBasisCents === 0 && totals.budgeted > 0 ? <Link className="budget-zero-warning neutral" href={`/income?month=${format(monthDate, "yyyy-MM")}`}><TriangleAlert size={17} /> No income expected or received yet this month. Add expected income to plan against it.</Link> : available < 0 ? <div className="budget-zero-warning"><TriangleAlert size={17} /> Assigned exceeds {incomeBasisLabel} income by {formatCurrency(Math.abs(available))}.</div> : null}
    ...
    {editingBudget ? <BudgetEditor categories={visible} incomeCents={incomeBasisCents} month={initialWorkspace.month} onClose={() => setEditingBudget(false)} onSave={saveMonthlyBudgets} /> : null}
...
function CategoryPanel({ allCategories, category, categoryGroups, month, message, onClose, onMoveMoney, onSaveCategory, onSaveBudget }: { allCategories: BudgetCategory[]; category: BudgetCategory; categoryGroups: string[]; month: string; message: string; onClose: () => void; onMoveMoney: () => void; onSaveCategory: (value: BudgetCategory) => Promise<boolean>; onSaveBudget: (id: string, cents: number) => Promise<boolean> }) {
  ...
  return <CategoryDetail actions={<><button className="primary-button" onClick={() => setMode(mode === "edit" ? "view" : "edit")}>{mode === "edit" ? "Close editor" : "Edit monthly amount"}</button><button className="secondary-button" onClick={onMoveMoney}><ArrowLeftRight size={16} /> Move money</button></>} allCategories={allCategories} category={category} message={message} month={month} onClose={onClose}>
