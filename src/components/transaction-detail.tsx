...
export function TransactionDetail({ transaction, categories, onClose, onUpdated }: { transaction: ActivityTransaction; categories: BudgetCategory[]; onClose: () => void; onUpdated: (transaction: ActivityTransaction) => void }) {
  ...
  function close() { if (!dirty || window.confirm("Discard unsaved transaction changes?")) onClose(); }
  ...
  return <>
    <BottomSheet className="transaction-sheet" label={`${transaction.merchant} transaction details`} onClose={close} handleLabel="Drag down to close transaction details">
      <div className="transaction-detail-header"><button className="icon-button" onClick={close} aria-label="Close"><X /></button><span className={`status-badge ${transaction.status}`}>{transaction.status}</span>{transaction.auditHistory.some((event) => event.action !== "undone") ? <button className="undo-button" disabled={saving} onClick={undo}><RotateCcw size={16} /> Undo</button> : <span />}</div>
