...
export function TransactionDetail({ transaction, categories, onClose, onUpdated }: { transaction: ActivityTransaction; categories: BudgetCategory[]; onClose: () => void; onUpdated: (transaction: ActivityTransaction) => void }) {
  ...
  return <div className="sheet-backdrop" onPointerDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><section ref={sheetRef} tabIndex={-1} role="dialog" aria-modal="true" className="transaction-sheet" style={{ transform: `translateY(${dragY}px)` }} aria-label={`${transaction.merchant} transaction details`}>
    ...
    <div className="transaction-hero"><h2>{displayName}</h2><strong className={transaction.amountCents > 0 ? "income" : ""}>{formatCurrency(transaction.amountCents, { signed: true })}</strong><p>{format(new Date(transaction.isoDate), "EEEE, MMMM d, yyyy")}</p></div>
    <div className="field"><label htmlFor="detail-display-name">Merchant display name</label><input id="detail-display-name" value={displayName} onChange={(event) => setDisplayName(event.target.value)} /></div><dl className="transaction-facts"><div><dt>Account</dt><dd>{transaction.accountName}</dd></div><div><dt>Imported description</dt><dd>{transaction.rawDescription || "No provider description"}</dd></div><div><dt>Review</dt><dd>{reviewed ? "Reviewed" : "Needs review"}</dd></div></dl>
