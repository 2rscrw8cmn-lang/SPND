...
export async function POST(request: Request) {
  ...
  const reconciled = !error && data ? await attemptIncomeReconciliation(supabase, auth.householdId, auth.userId) : false;
  return NextResponse.json({ message: error ? "Expected income could not be added." : reconciled ? "Expected income added." : "Expected income added. Occurrences will finish syncing shortly.", id: data?.id }, { status: error ? 500 : 200 });
}
