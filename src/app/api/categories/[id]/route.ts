...
const schema = z.object({ name: z.string().trim().min(1).max(60), color: z.string().regex(/^#[0-9A-Fa-f]{6}$/), icon: z.string().min(1).max(40), categoryGroup: z.enum(["Essentials", "Lifestyle", "Goals", "Excluded"]), isActive: z.boolean(), isExcluded: z.boolean(), showInBudget: z.boolean() });
