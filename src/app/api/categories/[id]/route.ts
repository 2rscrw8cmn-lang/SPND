...
const schema = z.object({ name: z.string().trim().min(1).max(60), color: z.string().regex(/^#[0-9A-Fa-f]{6}$/), icon: z.string().min(1).max(40), categoryGroup: z.string().trim().min(1).max(40), isActive: z.boolean(), isExcluded: z.boolean(), showInBudget: z.boolean() });
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const excluded = body.data.categoryGroup === "Excluded" || body.data.isExcluded;
  const { error } = await supabase.from("categories").update({ name: body.data.name, color: body.data.color, icon: body.data.icon, category_group: body.data.categoryGroup, is_active: body.data.isActive, is_excluded: excluded, show_in_budget: excluded || body.data.categoryGroup === "Income" ? false : body.data.showInBudget, updated_at: new Date().toISOString() }).eq("id", id).eq("household_id", auth.householdId);
  ...
  await supabase.from("audit_events").insert({ household_id: auth.householdId, actor_user_id: auth.userId, entity_type: "category", entity_id: id, action: body.data.isActive ? "updated" : "archived", metadata: { beforeName: existing.name, name: body.data.name, categoryGroup: body.data.categoryGroup, isActive: body.data.isActive } });
