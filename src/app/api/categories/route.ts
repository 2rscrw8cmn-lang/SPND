...
import { z } from "zod";
import { inferCategoryColor, inferCategoryIcon } from "@/lib/category-style";
...
export async function POST(request: Request) {
  ...
  const behaviorType = excluded ? "excluded" : body.data.categoryGroup === "Income" ? "income" : body.data.categoryGroup === "Goals" ? "goal" : "spending";
  const { data, error } = await supabase.from("categories").insert({ household_id: auth.householdId, name: body.data.name, color: inferCategoryColor(body.data.name), icon: inferCategoryIcon(body.data.name), category_group: body.data.categoryGroup, behavior_type: behaviorType, is_active: true, is_excluded: excluded, show_in_budget: !excluded && body.data.categoryGroup !== "Income", sort_order: 500 }).select("id,name,color,icon,category_group,behavior_type,is_active,is_excluded,show_in_budget").single();
  ...
  return NextResponse.json({ message: "Category added.", category: { id: data.id, name: data.name, color: data.color, icon: data.icon, categoryGroup: data.category_group, behaviorType: data.behavior_type, isActive: data.is_active, isExcluded: data.is_excluded, showInBudget: data.show_in_budget, budgetedCents: 0, spentCents: 0, pendingCents: 0, recentTransactions: [] } });
}
