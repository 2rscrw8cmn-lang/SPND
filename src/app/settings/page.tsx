...
import { getBudgetWorkspace, getConnectionHealth, getExpectedIncomeSources, getHouseholdSummary } from "@/lib/data";
...
export default async function SettingsPage() {
  const [health, workspace, household, incomeSources, merchantRules] = await Promise.all([getConnectionHealth(), getBudgetWorkspace(), getHouseholdSummary(), getExpectedIncomeSources(), getMerchantRules()]);
  ...
  return <PageShell>
    ...
    <section className="settings-section"><HouseholdSettings initialHousehold={household} /></section>

    <section className="settings-section"><div className="section-heading"><div><h2>Budget planning</h2><p>Maintain expected income, automation, and categories used by the monthly plan.</p></div></div><ExpectedIncomeSettings initialSources={incomeSources} /><MerchantRuleSettings initialRules={merchantRules} categories={workspace.categories} /><CategoryGroupSettings initialGroups={workspace.categoryGroups} categoryCounts={categoryCounts} /></section>

    <section className="settings-section"><div className="section-heading"><div><h2>Accounts & data</h2><p>Control connected accounts and verify household data health.</p></div></div><div className="settings-link-card card"><a className="settings-row" href="/settings/accounts"><span className="settings-row-icon"><Landmark size={19} /></span><div><strong>Accounts</strong><span>Roles, balance treatment, and net worth</span></div></a>{importsEnabled() ? <a className="settings-row" href="/settings/imports"><span className="settings-row-icon"><FileUp size={19} /></span><div><strong>Experimental import inbox</strong><span>Controlled testing only</span></div></a> : null}<a className="settings-row" href="/settings/diagnostics"><span className="settings-row-icon"><ClipboardCheck size={19} /></span><div><strong>Data health</strong><span>Check balances, transactions, and allocations</span></div></a></div></section>
    ...
    <section className="settings-section"><div className="section-heading"><div><h2>App</h2><p>Install SPND for a full-screen, home-screen experience.</p></div></div><InstallAppSettings /></section>
  </PageShell>;
}
