...
import {
  resolveCategoryIcon,
  resolveCategoryPaletteKey,
} from "@/lib/category-style";
import { calculateSafeToSpnd } from "@/lib/safe-to-spnd";
import {
  incomeOccurrencesForMonth,
  receivedIncomeTotal,
  type IncomeSchedule,
} from "@/lib/expected-income";
...
export type BudgetWorkspace = {
  ...
  monthSetup: {
    ...
    templateCategoryCount: number;
    templateTotalCents: number;
  };
};
...
export type IncomeViewData = {
  ...
  remainingCents: number;
  upcoming: BudgetWorkspace["expectedIncome"];
  received: ReceivedIncomeTransaction[];
  unmatched: ActivityTransaction[];
  openExpectations: OpenIncomeExpectation[];
};
...
async function householdContext() {
  ...
  const { data: membership } = await supabase
    ...
    .maybeSingle();
  return membership
    ? { supabase, householdId: membership.household_id as string }
    : null;
}
...
export async function getBudgetWorkspace(
  ...
  if (isDemoMode) {
    ...
    const categories = demoCategories.map((category) => ({
      ...
      recentTransactions: isCurrentMonth
        ? demoTransactions
            .filter(
              (transaction) =>
    ...
    const budgetedCents = categories.reduce(
      (sum, item) => sum + item.budgetedCents,
      0,
    );
    const spentCents = categories.reduce(
      (sum, item) => sum + item.spentCents,
      0,
    );
    const pendingCents = categories.reduce(
      (sum, item) => sum + item.pendingCents,
      0,
    );
    const expectedIncomeCents = isCurrentMonth ? 685000 : 0;
    const receivedIncomeCents = isCurrentMonth ? 342500 : 0;
    return {
      month,
      categories,
      categoryGroups: demoCategoryGroups,
      unsortedCount: isCurrentMonth ? 1 : 0,
      unsortedCents: isCurrentMonth ? 7421 : 0,
      expectedIncome: isCurrentMonth
        ? [
            {
              sourceId: "demo-income",
              name: "Paycheck",
            ...
            },
            {
              sourceId: "demo-income",
              name: "Paycheck",
              date: format(addMonths(new Date(), 0), "yyyy-MM-28"),
              amountCents: 342500,
            },
          ]
        : [],
      monthSetup: {
        previousCategoryCount: 6,
        previousTotalCents: 517500,
        templateCategoryCount: 6,
        templateTotalCents: 517500,
      },
      totals: {
        expectedIncomeCents,
        receivedIncomeCents,
        remainingExpectedIncomeCents: Math.max(
          0,
          expectedIncomeCents - receivedIncomeCents,
        ),
        budgetedCents,
        leftToAssignCents: expectedIncomeCents - budgetedCents,
        spentCents,
        pendingCents,
        remainingCents: budgetedCents - spentCents - pendingCents,
      },
    };
  }
  ...
  const [
    ...
    context.supabase
      .from("categories")
      .select(
        "id,name,color,icon,palette_key,category_group,is_active,is_excluded,show_in_budget,behavior_type",
      )
      .eq("household_id", context.householdId)
      .order("sort_order"),
    context.supabase
    ...
    context.supabase
      ...
      .lt("date", format(addMonths(monthDate, 1), "yyyy-MM-dd")),
  ]);
  const { data: transactions } = await context.supabase
    .from("transactions")
    .select("id,status,amount_cents,merchant,transacted_at,review_status")
    .eq("household_id", context.householdId)
    .eq("excluded", false)
    .is("superseded_by_transaction_id", null)
    .or(
      `and(status.eq.posted,posted_at.gte.${monthStart},posted_at.lt.${nextMonthStart}),and(status.eq.pending,transacted_at.gte.${monthStart},transacted_at.lt.${nextMonthStart})`,
    );
  ...
  const { data: allocations } = transactionIds.length
    ? await context.supabase
        ...
        .select("transaction_id,category_id,amount_cents,source")
  ...
  for (const allocation of allocations ?? []) {
    ...
    const transaction = transactionById.get(
      allocation.transaction_id as string,
    );
    if (transaction && Number(transaction.amount_cents) < 0) {
      const amount = Math.abs(Number(allocation.amount_cents));
      const target =
        statusById.get(allocation.transaction_id as string) === "pending"
          ? pending
          : spent;
      target.set(
        allocation.category_id as string,
        (target.get(allocation.category_id as string) ?? 0) + amount,
      );
      ...
      list.push({
        ...
        allocationSource: (allocation.source ??
          null) as AllocationSource | null,
      });
      recentByCategory.set(allocation.category_id as string, list);
    }
  }
  const matchedTransactionIds = (matchedPlanItems ?? [])
    .map((item) => item.matched_transaction_id as string | null)
    .filter((id): id is string => Boolean(id));
  const { data: matchedTransactions } = matchedTransactionIds.length
    ? await context.supabase
        .from("transactions")
        .select("id,excluded,is_transfer")
        .in("id", matchedTransactionIds)
    : { data: [] };
  const matchedTransfers = new Set(
    (matchedTransactions ?? [])
      .filter((transaction) => transaction.excluded || transaction.is_transfer)
      .map((transaction) => transaction.id as string),
  );
  const behaviorByRawCategory = new Map(
    (categoryRows ?? []).map((category) => [
      category.id as string,
      category.behavior_type as string,
    ]),
  );
  for (const item of matchedPlanItems ?? [])
    if (
      item.category_id &&
      item.matched_transaction_id &&
      matchedTransfers.has(item.matched_transaction_id as string) &&
      behaviorByRawCategory.get(item.category_id as string) === "goal"
    )
      spent.set(
        item.category_id as string,
        (spent.get(item.category_id as string) ?? 0) +
          Math.abs(Number(item.amount_cents)),
      );
  const budgetByCategory = new Map(
    (budgets ?? []).map((budget) => [
      budget.category_id as string,
      Number(budget.budgeted_cents),
    ]),
  );
  const unsorted = (transactions ?? []).filter(
    (transaction) =>
      Number(transaction.amount_cents) < 0 &&
      !allocatedIds.has(transaction.id as string),
  );
  const categories = (categoryRows ?? [])
    ...
    .map((category) => ({
      ...
      icon: resolveCategoryIcon(
        category.icon as string | null,
        category.name as string,
      ),
      paletteKey: resolveCategoryPaletteKey(
        category.palette_key as string | null,
        category.name as string,
      ),
      ...
      recentTransactions: (recentByCategory.get(category.id as string) ?? [])
  ...
  const categoryGroups = groupRows?.length
    ...
    : fallbackGroupNames.map((name, index) => ({
        id: `legacy-${name.toLowerCase().replaceAll(/[^a-z0-9]+/g, "-")}`,
        name,
        sortOrder: name === "Excluded" ? 90 : (index + 1) * 10,
        isSystem: true,
      }));
  ...
  const budgetedCents = visible.reduce(
    (sum, item) => sum + item.budgetedCents,
    0,
  );
  const spentCents = visible.reduce((sum, item) => sum + item.spentCents, 0);
  const pendingCents = visible.reduce(
    (sum, item) => sum + item.pendingCents,
    0,
  );
  ...
  const receivedIncomeCents = receivedIncomeTotal(
    (transactions ?? []).map((transaction) => ({
      id: transaction.id as string,
    ...
    (allocations ?? []).map((allocation) => ({
      transactionId: allocation.transaction_id as string,
  ...
  const incomeSources = (incomeRows ?? []).map((source) => ({
    ...
    cadence: source.cadence as string | null,
    explicitDates: (source.explicit_dates as string[] | null) ?? [],
    nextExpectedDate: source.next_expected_date as string | null,
    active: Boolean(source.active),
    sourceType: source.source_type as "recurring" | "one_time",
  }));
  const expectedIncome = incomeOccurrencesForMonth(incomeSources, month);
  const expectedIncomeCents = expectedIncome.reduce(
  ...
  return {
    ...
    expectedIncome,
    monthSetup: {
      previousCategoryCount: previousBudgets?.length ?? 0,
      previousTotalCents: (previousBudgets ?? []).reduce(
        (sum, item) => sum + Number(item.budgeted_cents),
        0,
      ),
      templateCategoryCount: templates?.length ?? 0,
      templateTotalCents: (templates ?? []).reduce(
        (sum, item) => sum + Number(item.budgeted_cents),
        0,
      ),
    },
    unsortedCount: unsorted.length,
    unsortedCents: unsorted.reduce(
      (sum, item) => sum + Math.abs(Number(item.amount_cents)),
      0,
    ),
    totals: {
      expectedIncomeCents,
...
export async function getExpectedIncomeSources(): Promise<
    ...
    return [
      {
        id: "demo-income",
        name: "Paycheck",
        expectedAmountCents: 342500,
        cadence: "biweekly",
        explicitDates: [],
        nextExpectedDate: format(new Date(), "yyyy-MM-dd"),
        active: true,
...
export async function getIncomeView(
  ...
  if (isDemoMode) {
    ...
    return {
      ...
      expectedCents: workspace.totals.expectedIncomeCents,
      receivedCents,
...
export async function getNextExpectedIncome(): Promise<{
  date: string;
  amountCents: number;
  name: string;
} | null> {
  if (isDemoMode)
    return {
      date: demoPlan[0]!.date,
      amountCents: demoPlan[0]!.amountCents,
      name: demoPlan[0]!.name,
    };
  const context = await householdContext();
  ...
  const { data } = await context.supabase
    .from("expected_income_sources")
    .select(
      "id,name,expected_amount_cents,cadence,explicit_dates,next_expected_date,active,source_type",
    )
    .eq("household_id", context.householdId)
    .eq("active", true);
...
export async function getMerchantRules() {
  ...
  const { data } = await context.supabase
    .from("merchant_rules")
    .select("id,normalized_merchant,category_id,active")
    .eq("household_id", context.householdId)
...
export async function getHouseholdSummary(): Promise<HouseholdSummary> {
  if (isDemoMode)
    return {
      name: "Turco Household",
      timezone: "America/New_York",
      minimumCashBufferCents: 75000,
      memberCount: 2,
    };
  const context = await householdContext();
  if (!context)
  ...
  const [{ data: household }, { count }] = await Promise.all([
    context.supabase
      .from("households")
      .select("name,timezone,minimum_cash_buffer_cents")
...
export async function getBudgetData(monthValue?: string) {
  return (await getBudgetWorkspace(monthValue)).categories.filter(
    (category) =>
      category.isActive && category.showInBudget && !category.isExcluded,
  );
}
...
export async function getAccountsData() {
    ...
    return [
      {
        ...
        balanceCents: 492500,
        availableBalanceCents: 492500,
        balanceAsOf: new Date().toISOString(),
        role: "cash" as const,
        payInFull: false,
        liabilityBalanceSign: null,
        balanceBasis: "available" as const,
        pendingTransactionsInBalance: true,
      ...
      },
      {
        ...
        balanceAsOf: new Date().toISOString(),
        role: "credit_card" as const,
        payInFull: true,
        liabilityBalanceSign: -1 as const,
        balanceBasis: "current" as const,
        pendingTransactionsInBalance: true,
        creditCardDueDate: null,
      },
    ];
  ...
  const { data } = await context.supabase
    .from("accounts")
    .select(
      "id,name,institution_name,current_balance_cents,available_balance_cents,balance_as_of,account_role,credit_card_pay_in_full,liability_balance_sign,balance_basis_state,pending_transactions_in_balance,credit_card_due_date",
    )
    .eq("household_id", context.householdId)
    .order("name");
  return (data ?? []).map((account) => ({
    ...
    institutionName:
      (account.institution_name as string | null) ?? "Imported account",
    balanceCents: Number(account.current_balance_cents),
    availableBalanceCents:
      account.available_balance_cents === null
        ? null
        : Number(account.available_balance_cents),
    balanceAsOf: account.balance_as_of as string | null,
    role: account.account_role as
      "cash" | "credit_card" | "investment" | "other_liability" | "excluded",
    payInFull: Boolean(account.credit_card_pay_in_full),
    liabilityBalanceSign: account.liability_balance_sign as -1 | 1 | null,
    balanceBasis: account.balance_basis_state as
      "needs_review" | "current" | "available",
    pendingTransactionsInBalance: account.pending_transactions_in_balance as
      boolean | null,
    creditCardDueDate: account.credit_card_due_date as string | null,
  }));
}
...
export async function getConnectionHealth(): Promise<ConnectionHealth> {
  if (isDemoMode)
    return {
      status: "active",
      lastSuccessfulSync: new Date(Date.now() - 18 * 60_000).toISOString(),
      lastAttemptedSync: new Date(Date.now() - 18 * 60_000).toISOString(),
      accountCount: 2,
      transactionCount: 24,
      lastResult: "success",
      sanitizedError: null,
  ...
  const { data } = await context.supabase
    ...
    .select(
      "status,last_synced_at,last_attempted_at,account_count,last_transaction_count,last_sync_status,sanitized_error,last_error",
    )
    .eq("household_id", context.householdId)
  ...
  return {
    status: data.status as ConnectionHealth["status"],
    lastSuccessfulSync: data.last_synced_at as string | null,
    lastAttemptedSync: data.last_attempted_at as string | null,
    accountCount: Number(data.account_count ?? 0),
    transactionCount: Number(data.last_transaction_count ?? 0),
    lastResult: data.last_sync_status as string | null,
    sanitizedError: (data.sanitized_error ?? data.last_error) as string | null,
  };
}
...
export async function getImportInbox(): Promise<ImportInboxItem[]> {
    ...
    return [
      {
        ...
        fileName: "august-budget.csv",
        importType: "budget_template",
        status: "ready",
  ...
  const { data: imports } = await context.supabase
    .from("imports")
    .select(
      "id,file_name,import_type,status,created_at,accepted_rows,duplicate_rows,review_rows",
    )
    .eq("household_id", context.householdId)
    .order("created_at", { ascending: false })
    .limit(25);
  const ids = (imports ?? []).map((item) => item.id as string);
  const [{ data: rows }, { data: errors }] = ids.length
    ? await Promise.all([
        context.supabase
          .from("import_rows")
        ...
        context.supabase
          .from("import_errors")
          .select("import_id,import_row_id,message,resolved_at")
          .in("import_id", ids)
          .is("resolved_at", null),
      ])
    : [{ data: [] }, { data: [] }];
  return (imports ?? []).map((item) => ({
    ...
    importType: item.import_type as string,
    status: item.status as string,
    createdAt: item.created_at as string,
    acceptedRows: Number(item.accepted_rows),
    duplicateRows: Number(item.duplicate_rows),
    reviewRows: Number(item.review_rows),
    errors: (errors ?? [])
      .filter((error) => error.import_id === item.id && !error.import_row_id)
      .map((error) => error.message as string),
    rows: (rows ?? [])
      .filter((row) => row.import_id === item.id)
      .map((row) => ({
        id: row.id as string,
        ...
        errors: (errors ?? [])
          .filter((error) => error.import_row_id === row.id)
          .map((error) => error.message as string),
      })),
  }));
}
...
export async function getReconciliationData() {
  if (isDemoMode)
    return {
      ...
      excludedAccounts: 0,
      checksRunAt: new Date().toISOString(),
    };
  const context = await householdContext();
  if (!context)
    return {
  ...
  const [{ data: accounts }, { data: transactions }, { data: allocations }] =
    await Promise.all([
      context.supabase
        .from("accounts")
        .select("id,balance_as_of,cash_flow_mode")
        .eq("household_id", context.householdId),
      context.supabase
        .from("transactions")
        .select("id,amount_cents,status,excluded,superseded_by_transaction_id")
        .eq("household_id", context.householdId),
      context.supabase
        .from("transaction_allocations")
  ...
  for (const allocation of allocations ?? [])
    allocationTotals.set(
      allocation.transaction_id as string,
      (allocationTotals.get(allocation.transaction_id as string) ?? 0) +
  ...
  return {
    ...
    staleAccounts: (accounts ?? []).filter(
      (account) =>
        !account.balance_as_of || account.balance_as_of < staleBefore,
    ).length,
    activeTransactions: (transactions ?? []).filter(
      (transaction) => !transaction.superseded_by_transaction_id,
    ).length,
    supersededPending: (transactions ?? []).filter(
      (transaction) =>
        transaction.status === "pending" &&
        transaction.superseded_by_transaction_id,
    ).length,
    allocationMismatches: expenses.filter(
      (transaction) =>
        (allocationTotals.get(transaction.id as string) ?? 0) !==
        Number(transaction.amount_cents),
    ).length,
    excludedAccounts: (accounts ?? []).filter(
      (account) => account.cash_flow_mode === "excluded",
    ).length,
    checksRunAt: new Date().toISOString(),
  };
}
...
export async function getActivityData(
  ...
  const inSelectedMonth = (isoDate: string) =>
    !monthDate ||
    (new Date(isoDate) >= monthDate &&
      new Date(isoDate) < addMonths(monthDate, 1));
  if (isDemoMode)
    return demoTransactions
      .filter((transaction) => inSelectedMonth(transaction.isoDate))
      .map((transaction) => ({
        ...transaction,
        updatedAt: transaction.isoDate,
        reviewStatus: transaction.excluded
          ? ("reviewed" as const)
          : transaction.reviewStatus,
        importedMerchant: transaction.merchant,
        allocationSource:
          transaction.id === "t1"
            ? ("merchant_rule" as const)
        ...
        auditHistory:
          transaction.id === "t1"
            ? [
                {
                  id: "demo-audit",
                  action: "Imported from SimpleFIN",
                  createdAt: transaction.isoDate,
                  undoable: false,
                },
              ]
            : [],
  ...
  let query = context.supabase
    .from("transactions")
    .select(
      "id,merchant,display_name,amount_cents,status,transacted_at,posted_at,raw_description,note,excluded,is_transfer,is_recurring,review_status,reviewed_at,updated_at,account_id,accounts(name),transaction_allocations(category_id,amount_cents,source,categories(name,color))",
    )
    .eq("household_id", context.householdId)
    .is("superseded_by_transaction_id", null);
  if (monthDate)
    query = query
      ...
      .lt("transacted_at", addMonths(monthDate, 1).toISOString());
  ...
  if (options.date)
    query = query
      .gte("transacted_at", `${options.date}T00:00:00`)
      .lt("transacted_at", `${options.date}T23:59:59.999`);
  if (options.query?.trim()) {
    const term = options.query.trim().replaceAll(/[,%()]/g, " ");
    query = query.or(
      `merchant.ilike.%${term}%,display_name.ilike.%${term}%,raw_description.ilike.%${term}%`,
    );
  }
  if (options.filter === "needs_review")
    query = query.eq("review_status", "needs_review").eq("excluded", false);
  ...
  if (options.categoryId) {
    const { data: categoryAllocations } = await context.supabase
      .from("transaction_allocations")
      .select("transaction_id")
      .eq("household_id", context.householdId)
      .eq("category_id", options.categoryId)
      .limit(5000);
    const ids = (categoryAllocations ?? []).map(
      (item) => item.transaction_id as string,
    );
  ...
  const { data: auditEvents } = transactionIds.length
    ? await context.supabase
        .from("audit_events")
        .select("id,entity_id,action,created_at,metadata")
        .eq("household_id", context.householdId)
  ...
  return (data ?? []).map((transaction) => {
    const allocations =
      transaction.transaction_allocations as unknown as Array<{
        category_id: string;
        amount_cents: number;
    ...
    return {
      id: transaction.id as string,
      merchant:
        (transaction.display_name as string | null) ||
      ...
      reviewStatus: transaction.excluded
        ? "reviewed"
        : (transaction.review_status as "needs_review" | "reviewed"),
      ...
      allocations: (allocations ?? []).map((allocation) => ({
        ...
        category: allocation.categories?.name ?? "Unsorted",
        amountCents: Number(allocation.amount_cents),
      })),
      auditHistory: (auditEvents ?? [])
        .filter((event) => event.entity_id === transaction.id)
        .map((event) => ({
          id: event.id as string,
          action: String(event.action).replaceAll("_", " "),
          createdAt: event.created_at as string,
          undoable:
            Boolean((event.metadata as { before?: unknown } | null)?.before) &&
            event.action !== "undone",
        })),
    };
  });
...
export async function getPlanData() {
  ...
  return [
    ...
    ...(planned ?? []).map((item) => ({
      ...
      matchedTransactionId: item.matched_transaction_id as string | null,
    })),
...
export async function getRecurringCandidates() {
  ...
  const { data } = await context.supabase
    .from("recurring_items")
    .select("id,name,amount_cents,next_due_date,type")
    .eq("household_id", context.householdId)
...
export async function getSafeBreakdown() {
  ...
  const [
    ...
    context.supabase
      .from("accounts")
      .select(
        "id,name,account_role,current_balance_cents,available_balance_cents,balance_as_of,balance_basis_state,pending_transactions_in_balance,credit_card_pay_in_full,liability_balance_sign",
      )
      .eq("household_id", context.householdId),
    context.supabase
      .from("transactions")
    ...
    context.supabase
      .from("households")
      .select("minimum_cash_buffer_cents,timezone")
      .eq("id", context.householdId)
      .single(),
    context.supabase
      .from("planned_items")
    ...
    context.supabase
      .from("recurring_items")
      .select(
        "id,name,next_due_date,amount_cents,type,category_id,state,matched_transaction_id",
      )
      .eq("household_id", context.householdId)
      .in("state", ["confirmed", "matched"]),
    context.supabase
      .from("expected_income_sources")
      .select(
        "id,name,expected_amount_cents,cadence,next_expected_date,explicit_dates,source_type,active",
      )
      .eq("household_id", context.householdId)
      .eq("active", true),
  ]);
  ...
  for (const transaction of pendingResult.data ?? [])
    pendingByAccount.set(
      transaction.account_id as string,
      (pendingByAccount.get(transaction.account_id as string) ?? 0) +
        Number(transaction.amount_cents),
    );
  ...
  const safeIncomeSources = (incomeResult.data ?? []).map((source) => ({
    id: source.id as string,
    name: source.name as string,
    expectedAmountCents: Number(source.expected_amount_cents),
    cadence: source.cadence as string | null,
  ...
  const scheduleDates = [
    today.slice(0, 7),
    format(addMonths(new Date(`${today}T12:00:00Z`), 1), "yyyy-MM"),
  ].flatMap((month) =>
    incomeOccurrencesForMonth(safeIncomeSources, month).map(
      (item) => item.date,
    ),
  );
  const incomeDates = [
  ...
  const reserves = planItems
    ...
    .map((item) => ({
      ...
      fulfilled:
        item.state === "matched" || Boolean(item.matched_transaction_id),
      behavior: categoryBehavior.get(item.category_id as string),
    }));
  return calculateSafeToSpnd({
    accounts: (accountsResult.data ?? []).map((account) => ({
      id: account.id as string,
      name: account.name as string,
      role: account.account_role as import("@/lib/safe-to-spnd").AccountRole,
      currentBalanceCents: Number(account.current_balance_cents),
      availableBalanceCents:
        account.available_balance_cents === null
          ? null
          : Number(account.available_balance_cents),
      balanceAsOf: account.balance_as_of as string | null,
      balanceBasis:
        account.balance_basis_state as import("@/lib/safe-to-spnd").BalanceBasisState,
      pendingTransactionsInBalance: account.pending_transactions_in_balance as
        boolean | null,
      pendingTransactionCents: pendingByAccount.get(account.id as string) ?? 0,
      payInFull: Boolean(account.credit_card_pay_in_full),
      liabilityBalanceSign: account.liability_balance_sign as -1 | 1 | null,
    })),
    ...
    spendingCategories: categories
      .filter((category) => category.behaviorType === "spending")
      .map((category) => ({
        id: category.id,
        name: category.name,
        monthlyBudgetCents: category.budgetedCents,
        postedSpentCents: category.spentCents,
        pendingSpentCents: category.pendingCents,
      })),
    minimumBufferCents: Number(
      householdResult.data?.minimum_cash_buffer_cents ?? 0,
    ),
    today,
